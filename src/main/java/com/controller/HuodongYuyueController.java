...
, "ganbuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 干部
            //级联表
            GanbuEntity ganbu = ganbuService.selectById(huodongYuyue.getGanbuId());
            if(ganbu != null){
    ...
    */
    ...
    public R save(@RequestBody HuodongYuyueEntity huodongYuyue, HttpServletRequest request){
        ...
        if(false)
            ...
            huodongYuyue.setGanbuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<HuodongYuyueEntity> queryWrapper = new EntityWrapper<HuodongYuyueEntity>()
            .eq("yonghu_id", huodongYuyue.getYonghuId())
            .eq("ganbu_id", huodongYuyue.getGanbuId())
...
//        else if("干部".equals(role))
//            huodongYuyue.setGanbuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(huodongYuyue.getHuodongYuyueFile()) || "null".equals(huodongYuyue.getHuodongYuyueFile())){
                huodongYuyue.setHuodongYuyueFile(null);
        }
...
//                            huodongYuyueEntity.setGanbuId(Integer.valueOf(data.get(0)));   //干部 要改的
...
//                            huodongYuyueEntity.setHuodongYuyueFile(data.get(0));                    //附件 要改的
